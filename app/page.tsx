...
const handleRegister = async (data: RegisterForm) => {
  ...
  return (
    <div style={{
      ...
      <div style={{
      ...
      }} className="md:flex lg:flex" id="left-panel">
        <div>
        ...
        <div style={{
          ...
          backgroundSize: '40px 40px', pointerEvents: 'none', overflow: 'hidden',
        }} />
      </div>
        ...
        <div style={{ width: '100%', maxWidth: '440px' }}>
            ...
            {(['login', 'register'] as const).map(m => (
              <button
                ...
                style={{
                  ...
                  background: mode === m ? 'linear-gradient(135deg, var(--accent-cyan)22, var(--accent-purple)22)' : 'transparent',
                  color: mode === m ? 'var(--accent-cyan)' : 'var(--text-muted)',
                  borderBottom: mode === m ? '1px solid var(--accent-cyan)' : '1px solid transparent',
                }}
              >
          ...
          {mode === 'login' ? (
            <form onSubmit={loginForm.handleSubmit(handleLogin)} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div>
                ...
                {loginForm.formState.errors.email && (
                  <p style={{ color: '#ef4444', fontSize: '12px', marginTop: '4px' }}>{loginForm.formState.errors.email.message}</p>
                )}
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '13px', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '6px' }}>
                ...
                <div style={{ position: 'relative' }}>
                  <input type={showPass ? 'text' : 'password'} placeholder="••••••••" {...loginForm.register('password')} style={{ paddingRight: '44px' }} />
                ...
                {loginForm.formState.errors.password && (
                  <p style={{ color: '#ef4444', fontSize: '12px', marginTop: '4px' }}>{loginForm.formState.errors.password.message}</p>
                )}
              </div>
            ...
            <form onSubmit={registerForm.handleSubmit(handleRegister)} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div>
                <label style={{ display: 'block', fontSize: '13px', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '6px' }}>Nom complet</label>
                <input type="text" placeholder="Alex Martin" {...registerForm.register('name')} />
                {registerForm.formState.errors.name && (
                  <p style={{ color: '#ef4444', fontSize: '12px', marginTop: '4px' }}>{registerForm.formState.errors.name.message}</p>
                )}
              </div>
              <div>
                ...
                {registerForm.formState.errors.email && (
                  <p style={{ color: '#ef4444', fontSize: '12px', marginTop: '4px' }}>{registerForm.formState.errors.email.message}</p>
                )}
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '13px', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '6px' }}>Mot de passe</label>
                <div style={{ position: 'relative' }}>
                  <input type={showPass ? 'text' : 'password'} placeholder="••••••••" {...registerForm.register('password')} style={{ paddingRight: '44px' }} />
                  <button type="button" onClick={() => setShowPass(!showPass)} style={{
                    position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)',
                    background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)',
                  }}>
                ...
                {registerForm.formState.errors.password && (
                  <p style={{ color: '#ef4444', fontSize: '12px', marginTop: '4px' }}>{registerForm.formState.errors.password.message}</p>
                )}
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '13px', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '6px' }}>Confirmer le mot de passe</label>
                <input type={showPass ? 'text' : 'password'} placeholder="••••••••" {...registerForm.register('confirmPassword')} />
                {registerForm.formState.errors.confirmPassword && (
                  <p style={{ color: '#ef4444', fontSize: '12px', marginTop: '4px' }}>{registerForm.formState.errors.confirmPassword.message}</p>
                )}
              </div>
              <button type="submit" className="btn-primary" style={{ marginTop: '8px', fontSize: '15px', padding: '13px' }}>
                Créer mon compte

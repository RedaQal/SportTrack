...
import { Target, TrendingUp, Award, Edit3, Check, X } from 'lucide-react';

const goalColors = ['var(--accent-cyan)', 'var(--accent-orange)', 'var(--accent-green)', 'var(--accent-purple)'];
...
export default function GoalsPage() {
  ...
  const handleEdit = (id: string, target: number) => {
    setEditing(id);
        ...
        {goalsWithProgress.map((goal, i) => (
          ...
          >
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '16px' }}>
              <div>
                <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '15px', marginBottom: '2px' }}>{goal.label}</div>
              ...
              {editing === goal.id ? (
                <div style={{ display: 'flex', gap: '6px' }}>
                  <button onClick={() => handleSave(goal.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--accent-green)' }}>
                    <Check size={16} />
                  </button>
                  <button onClick={() => setEditing(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)' }}>
                    <X size={16} />
                  </button>
                </div>
              ) : (
                <button onClick={() => handleEdit(goal.id, goal.target)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)' }}>
                  <Edit3 size={14} />
                </button>
              )}
            </div>
            ...
            <div style={{ marginBottom: '12px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', color: 'var(--text-secondary)', marginBottom: '6px' }}>
                <span style={{ fontWeight: 700, color: goalColors[i % 4], fontSize: '20px', fontFamily: 'var(--font-display)' }}>
              ...
              <div style={{ height: '8px', borderRadius: '4px', background: 'var(--bg-secondary)', overflow: 'hidden' }}>
                <div style={{
                  ...
                  width: `${goal.pct}%`,
                  background: `linear-gradient(90deg, ${goalColors[i % 4]}, ${goalColors[(i + 1) % 4]})`,
                  transition: 'width 0.6s ease',
                  boxShadow: `0 0 8px ${goalColors[i % 4]}66`,
                }} />
              </div>
            </div>
            ...
            <div style={{
              ...
              {goal.pct >= 100 ? (
                <span style={{ color: 'var(--accent-green)', fontWeight: 700 }}>✅ Objectif atteint !</span>
              ) : (
            ...
            {goal.pct >= 100 && (
              <div style={{
                ...
                borderRadius: '16px', border: `2px solid ${goalColors[i % 4]}`,
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '12px' }}>
          {badges.map(badge => (
            ...
            >
              ...
              {badge.earned && (
                <div style={{ marginTop: '8px', fontSize: '11px', color: 'var(--accent-green)', fontWeight: 700 }}>Débloqué ✓</div>
              )}
            </div>
          ))}
        </div>

        ...
        <div className="stat-card animate-fade-in" style={{ marginBottom: '20px' }}>
          ...
          <ResponsiveContainer width="100%" height={180}>
            <AreaChart data={trendData}>
              ...
              <Area type="monotone" dataKey="Calories" stroke="var(--accent-cyan)" fill="url(#colorCal)" strokeWidth={2} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      )}
        ...
        {[7, 30, 90].map(r => (
          <button
            ...
            style={{
              ...
              border: '1px solid', borderColor: range === r ? 'var(--accent-cyan)' : 'var(--border)',
        ...
        {['all', 'cardio', 'strength', 'hiit', 'sports', 'flexibility'].map(cat => (
          <button
            ...
            style={{
              ...
              fontFamily: 'var(--font-display)', fontWeight: 600, cursor: 'pointer',
              border: '1px solid',
              borderColor: filter === cat ? (categoryColors[cat] || 'var(--accent-cyan)') : 'var(--border)',
              background: filter === cat ? `${categoryColors[cat] || 'var(--accent-cyan)'}22` : 'transparent',
              color: filter === cat ? (categoryColors[cat] || 'var(--accent-cyan)') : 'var(--text-muted)',
            }}
          >
        ...
        {filtered.map(session => (
          ...
          >
            ...
            >
              ...
              {expanded === session.id ? <ChevronUp size={16} color="var(--text-muted)" /> : <ChevronDown size={16} color="var(--text-muted)" />}
            </div>
            ...
            {expanded === session.id && (
              <div style={{ padding: '0 16px 16px', borderTop: '1px solid var(--border)' }}>
                <div style={{ paddingTop: '12px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {session.exercises.map((e, i) => (
                    <div key={i} style={{
                      ...
                      <div style={{ textAlign: 'right' }}>
                        <div style={{ fontSize: '13px', fontWeight: 700, color: 'var(--accent-orange)' }}>{e.calories} kcal</div>
                        <div style={{ fontSize: '11px', padding: '2px 8px', borderRadius: '8px', background: `${categoryColors[e.category]}22`, color: categoryColors[e.category] }}>
                          {e.category}
                        </div>
                      </div>
                    </div>
                  ...
                  {session.notes && (
                    <div style={{ fontSize: '13px', color: 'var(--text-secondary)', padding: '10px 12px', borderRadius: '8px', background: 'var(--bg-secondary)', borderLeft: '3px solid var(--accent-cyan)' }}>
                      📝 {session.notes}
                    </div>
                  )}
                </div>

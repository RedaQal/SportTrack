...
export default function Dashboard() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      avgDuration: sessions.length ? Math.round(sessions.reduce((sum, s) => sum + s.totalDuration, 0) / sessions.length) : 0,
    };
  }, [sessions]);
  ...
  const weeklyData = useMemo(() => {
    ...
    return days.map(day => {
      ...
      return {
        ...
        Calories: daySessions.reduce((sum, s) => sum + s.totalCalories, 0),
        Durée: daySessions.reduce((sum, s) => sum + s.totalDuration, 0),
  ...
  const statCards = [
    { label: 'Calories cette semaine', value: stats.weekCalories.toLocaleString(), unit: 'kcal', icon: Flame, color: 'var(--accent-orange)', bg: 'rgba(255,107,53,0.1)' },
    { label: 'Durée cette semaine', value: stats.weekDuration, unit: 'min', icon: Clock, color: 'var(--accent-cyan)', bg: 'rgba(0,229,255,0.1)' },
    { label: 'Séances cette semaine', value: stats.weekWorkouts, unit: 'séances', icon: Dumbbell, color: 'var(--accent-green)', bg: 'rgba(0,255,136,0.1)' },
    { label: 'Durée moyenne', value: stats.avgDuration, unit: 'min/séance', icon: Activity, color: 'var(--accent-purple)', bg: 'rgba(124,58,237,0.1)' },
  ];
        ...
        <div className="stat-card animate-fade-in" style={{ animationDelay: '0.4s' }}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={weeklyData} barGap={4}>
              ...
              <Bar dataKey="Calories" fill="var(--accent-cyan)" radius={[4, 4, 0, 0]} opacity={0.85} />
              <Bar dataKey="Durée" fill="var(--accent-purple)" radius={[4, 4, 0, 0]} opacity={0.85} />
            </BarChart>
          </ResponsiveContainer>
          <div style={{ display: 'flex', gap: '16px', marginTop: '8px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', color: 'var(--text-muted)' }}>
              <div style={{ width: 10, height: 10, borderRadius: '2px', background: 'var(--accent-cyan)' }} />
              Calories
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', color: 'var(--text-muted)' }}>
              <div style={{ width: 10, height: 10, borderRadius: '2px', background: 'var(--accent-purple)' }} />
              Durée (min)
            </div>
          </div>
        </div>
